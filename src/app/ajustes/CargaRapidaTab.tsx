...
const STATUS_CONFIG = {
  new:  { label: 'Nuevo',     color: 'var(--success)', Icon: CheckCircle2 },
  skip: { label: 'Ya existe', color: 'var(--warning)', Icon: AlertCircle  },
};
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-strong)', margin: 0 }}>Carga Rápida</h3>
        ...
        {rows.length > 0 && (
          <button onClick={handleReset} style={{
            ...
            background: 'var(--neutral-04)', border: '1px solid var(--neutral-08)',
      ...
      {!processed && (
        <textarea
          ...
          style={{
            width: '100%', background: 'var(--neutral-02)',
            border: '1px dashed var(--neutral-10)', borderRadius: 8,
            color: rawText ? 'var(--text-default)' : 'var(--text-subtle)', fontSize: 13, padding: 16, resize: 'vertical', boxSizing: 'border-box',
      ...
      {rows.length > 0 && !processed && (
        <div style={{ background: 'var(--neutral-02)', border: '1px solid var(--neutral-06)', borderRadius: 8, padding: 16 }}>
              ...
              <thead>
                <tr>
                  {Array.from({ length: colCount }, (_, i) => (
                    <th key={i} style={{ padding: '6px 8px', textAlign: 'left', borderBottom: '1px solid var(--neutral-06)' }}>
                      <select
                        value={mapping[i] ?? 'ignore'}
                        onChange={e => setMapping(prev => ({ ...prev, [i]: e.target.value as CargaRole }))}
                        style={{
                          background: 'var(--surface-card)', color: 'var(--text-strong)', border: '1px solid var(--neutral-10)',
                ...
                {rows.slice(0, 8).map((row, ri) => (
                  <tr key={ri}>
                    {Array.from({ length: colCount }, (_, ci) => (
                      <td key={ci} style={{ padding: '5px 8px', color: 'var(--text-subtle)', borderBottom: '1px solid var(--neutral-03)', whiteSpace: 'nowrap', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {row.cells[ci] ?? ''}
                      </td>
                    ))}
                  </tr>
          ...
          <div style={{ marginTop: 16, display: 'flex', gap: 8, alignItems: 'center' }}>
            <button
              ...
              style={{
                ...
                padding: '6px 16px',
                background: canProcess ? 'var(--accent)' : 'var(--surface-muted)',
                color: canProcess ? 'var(--text-on-accent)' : 'var(--text-disabled)',
                border: canProcess
            ...
            {!canProcess && (
              <span style={{ fontSize: 12, color: 'var(--text-subtle)' }}>Asigná al menos CUIL o Apellido y Nombre</span>
            )}
          </div>
        </div>
      )}
      ...
      {results && summary && !saved && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            {[
              { key: 'new',  label: `${summary.new} nuevos`,            color: 'var(--success)' },
              { key: 'skip', label: `${summary.skip} ya existen (se omiten)`, color: 'var(--warning)' },
            ].map(s => (
              <div key={s.key} style={{ padding: '6px 14px', borderRadius: 6, background: 'var(--neutral-03)', border: `1px solid ${s.color}22`, fontSize: 12, fontWeight: 700, color: s.color }}>
                {s.label}
              </div>
            ))}
          </div>
          ...
          {/* Results table */}
          <div style={{ overflowX: 'auto', background: 'var(--neutral-02)', border: '1px solid var(--neutral-06)', borderRadius: 8 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--neutral-06)' }}>
                  <th style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--text-subtle)', fontWeight: 700 }}>Estado</th>
                  <th style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--text-subtle)', fontWeight: 700 }}>Cliente</th>
                  <th style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--text-subtle)', fontWeight: 700 }}>CUIL</th>
                  <th style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--text-subtle)', fontWeight: 700 }}>Cambios</th>
                </tr>
              </thead>
              <tbody>
                {results.map((res, i) => {
                  ...
                  return (
                    <tr key={i} style={{ borderBottom: '1px solid var(--neutral-03)' }}>
                      ...
                      <td style={{ padding: '8px 12px', color: 'var(--text-subtle)', fontFamily: 'var(--font-code)', fontSize: 11 }}>
                        {res.parsedData.cuil ?? res.existingRecord?.cuil ?? '—'}
                      </td>
                      <td style={{ padding: '8px 12px' }}>
                        {res.status === 'new' ? (
                          <span style={{ color: 'var(--text-subtle)', fontSize: 11 }}>Registro nuevo</span>
                        ) : (
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button
              ...
              style={{
                ...
                background: summary.new > 0 && !saving
                  ? 'linear-gradient(135deg, #16a34a 0%, var(--success) 100%)'
                  : 'var(--neutral-04)',
                color: summary.new > 0 && !saving ? 'var(--surface-card)' : 'var(--text-subtle)',
                border: summary.new > 0 && !saving
                  ? '1px solid rgba(74,222,128,0.4)'
                  : '1px solid var(--neutral-06)',
            ...
            <button
              ...
              style={{ padding: '8px 14px', background: 'transparent', border: '1px solid var(--neutral-08)', borderRadius: 6, color: 'var(--text-subtle)', fontSize: 12, cursor: 'pointer' }}
            >
              Volver a mapear
            </button>
          </div>
          ...
          {error && (
            <div style={{ padding: '10px 14px', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 6, color: 'var(--danger-strong)', fontSize: 12 }}>
              Error: {error}
            </div>
          )}
        </div>
      ...
      {saved && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: 40 }}>
          ...
          <p style={{ color: 'var(--success)', fontWeight: 700, fontSize: 16 }}>¡Carga completada!</p>
          <button onClick={handleReset} style={{ padding: '8px 20px', background: 'var(--neutral-05)', border: '1px solid var(--neutral-08)', borderRadius: 6, color: 'var(--text-default)', fontSize: 13, cursor: 'pointer' }}>
            Nueva carga
          </button>
        </div>
      )}
